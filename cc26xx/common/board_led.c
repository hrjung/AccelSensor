...
//#include <ti/mw/display/Display.h> //debug
...
//hrjung add for compile for 7ID module
//not for custom #define MODULE_CC26XX_7X7	1
 ...
 * LOCAL FUNCTIONS
 */
//void board_led_blinkTimeoutCB(void);
 ...
 * EXTERNAL VARIABLES
 */
//#define SBP_SHOW_RESULT_EVT				  0x0002

//extern void AccelSensor_clockHandler(UArg arg);
/*********************************************************************
 * LOCAL VARIABLES
 */
...
void Board_Led_control(board_led_type led, board_led_state state)
{
    ...
    {
        if(Util_isActive(&blinkClkStruct) == false)
        {
    ...
    {
        if(Util_isActive(&blinkClkStruct) == true)
        {
...
static void board_led_blinkTimeoutCB(UArg a0)
{
	...
	//Display_print2(dispHandle, 3, 0, "LED : sate=%d status=%d", ledStatus[0].state, ledStatus[0].status);
}
...
{
#if 1
	//unsigned int ledType = Board_MOTOR_ALARM;
	unsigned int ledType = Board_LED1;
#else
